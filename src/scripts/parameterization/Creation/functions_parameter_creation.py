# -*- coding: utf-8 -*-
"""
Created on Fri Sep  9 12:20:44 2022
...
#%%
def param_create(Names: list, Const: list = ['Default'], Min: list = ['Default'], 
    ...
    if os.path.exists(os.path.abspath(Outfile_path)) != True:
        try: # create the path if possible
            #os.mkdir(os.path.abspath(Outfile_path))                        
            pathlib.Path(os.path.abspath(Outfile_path)).mkdir(parents=True, exist_ok=True)
        except OSError as error:
            print(error)
                
        if os.path.exists(os.path.abspath(Outfile_path)) != True:
            wmsg = "`Outfile_path` is not valid. `Write` is being set to False."
            warnings.warn(wmsg)
            pwrite = False
    ...
    if pwrite == True:        
        if pout_type == 'tsv':
            try:
                poutfile = os.path.abspath(pout_path + '/' + pout_name + '.txt')
                print("Writing {} to file.".format(poutfile))
                pdf.to_csv(poutfile, sep="\t", header=True, index=False)
            except:
                wmsg = 'Warning: Program failed unexpectedly when writing to file. Skipping write.'
                warnings.warn(wmsg)
                pass
            ...
            print("Warning: Invalid output file format, Skipping write.")
...
#tmin = [0, 5, 10, 15]
#tmax = [10, 15, 20, 25]
#tsd = [0.25, 0.25, 0.25, 0.25]
#tmethod = ['const', 'seq', 'unif', 'norm']
#param_create(tnames, tconst, tmin, tmax, tsd, tmethod, Write=True, Outfile_path='..', Outfile_type='tsv')
# =============================================================================
